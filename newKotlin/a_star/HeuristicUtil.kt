...
object HeuristicUtil {
	...
	fun getMoveInfo(newStack: Stack, oldStack: Stack): String {
		return "Moves(${newStack.moves.size - oldStack.moves.size}): ${
	...
	fun getCostInfo(stacks: List<Stack>): Set<String> {
		if (stacks.isEmpty()) return emptySet()
		val min = stacks.minOf { it.currentCost }
		return stacks.filter { it.currentCost == min }.map(::getCostInfo).toSet()
	}
	...
	fun getStackInfo(stack: Stack, moves: Boolean = true): String =
		"A: ${stack.a.value}, B: ${stack.b.value}, Chunk: ${stack.chunk.minValue}-${stack.chunk.maxValue}, " +
				"${getCostInfo(stack)}, ${if (moves) "Moves(${stack.moves.size}): ${stack.moves}" else ""}"
}
