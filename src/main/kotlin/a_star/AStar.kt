...
class AStar {
	...
	fun sort(stack: Stack): Stack {
		...
		System.err.println("Moves(${newStack.moves.size - oldStack.moves.size}): ${newStack.moves.subList(oldStack.moves.size, newStack.moves.size)}")
	...
	private fun aStar(stack: Stack, excludedMoves: List<Move>, computeHeuristics: (Stack) -> Int): Stack {
		...
		while (openList.isNotEmpty()) {
			...
			if (excludedMoves.last() == Move.PA && currentStack.b.containsAll(currentStack.chunk.first.values) ||
	...
	private fun getBestStates(originalStack: Stack, excludedMoves: List<Move>, computeHeuristics: (Stack) -> Int): List<Stack> {
		...
		for (move in Move.entries.filter { !excludedMoves.contains(it) }) {
			...
			if (move == Move.PA) {
				...
				val currentChunk = originalStack.chunk.first
				if (topB == null || topB !in currentChunk.values) {
					continue // Do not allow PA if top of B is not in current chunk
				}
			}
			if (move in Move.duoMoves && (originalStack.a.size < 2 || originalStack.b.size < 2)) continue
			...
			System.err.println("Applied move: $move | Before: ${getStackInfo(originalStack)} | After: ${getStackInfo(currentStack)}")
